%% Driver file which will simulate Sens expression along a gradient
...
Nalleles = 2;       % number of alleles per cell
...
TF(numel(grade)).cell(Ncells).allele(Nalleles).Simulation = 0 ; 
tic % calculate run time

% add required external files for parallel processor to access
parpool('AttachedFiles',{'TwoStatePromoter.m','ParameterFile.m'});
...
for tt=1:length(grade) 
    ...
    parfor ii=1:Ncells % parfor opens parpool for the first time
        % store mRNA and protein numbers across sim_time for 2 alleles per cell
        [cell(ii).allele(1)] =TwoStatePromoter(sim_time, TF(tt));
        [cell(ii).allele(2)] =TwoStatePromoter(sim_time, TF(tt));
    end
